...
- `src/mcp.rs` — MCP handlers, delivery decisions, diffs, token accounting, and evidence recording.
- `src/tools.rs` — workspace-scoped read, create-only write, exact edit, truncation, and path safety.
- `src/commands.rs` — command allowlist, execution limits, normalization, and Cargo test projection.
- `src/store.rs` — in-memory calls and baselines, API snapshots, and shutdown serialization.
...
- `tests/opencode/` — isolated OpenCode fixture, scenario prompts, configuration, and smoke runner.
